...
func _ready() -> void:
	var level_size_in_cells = $Terrain.get_used_rect().size
	var tile_size_in_px = $Terrain.tile_set.tile_size
...
func get_appropriate_pos_for_enemy(enemy: Enemy) -> Vector2:
	...
	var local_pos = Vector2(
		clampf(
			local_pos_raw.x,
			$Terrain.tile_set.tile_size.x / 2, 
			size_in_px.x - $Terrain.tile_set.tile_size.x / 2),
		clampf(
			local_pos_raw.y, 
			$Terrain.tile_set.tile_size.y / 2, 
			size_in_px.y - $Terrain.tile_set.tile_size.y / 2))
	
	var enemy_cell_pos = $Terrain.local_to_map(local_pos)
	
	var ok_terrain_cell_pos = _find_closest_cell_not_on_water_nor_an_obstacle(enemy_cell_pos)
	var ok_terrain_cell = $Terrain.get_cell_tile_data(ok_terrain_cell_pos)
	var ok_obstacle_cell = $Obstacles.get_cell_tile_data(ok_terrain_cell_pos)
	...
	return to_global($Terrain.map_to_local(ok_terrain_cell_pos))
	
func _find_closest_cell_not_on_water_nor_an_obstacle(initial_pos: Vector2i, max_radius: int = 10) -> Vector2i:
	# Convert world position to cell coordinates
	var initial_terrain_cell = $Terrain.get_cell_tile_data(initial_pos)
	var initial_obstacle_cell = $Obstacles.get_cell_tile_data(initial_pos)
	var initial_decoration_cell = $Decorations.get_cell_tile_data(initial_pos)
	...
	for radius in range(1, max_radius):
		for x in range(-radius, radius + 1):
			for y in range(-radius, radius + 1):
				...
				if !$Terrain.get_used_rect().has_point(new_pos):
					continue
				
				var new_terrain_cell = $Terrain.get_cell_tile_data(new_pos)
				var new_obstacle_cell = $Obstacles.get_cell_tile_data(new_pos)
				var new_decorations_cell = $Decorations.get_cell_tile_data(new_pos)
